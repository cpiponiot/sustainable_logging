...
For each scenario we simulated 1000 years of logging cycles, and determined the duration of sustained production $tsust$, i.e. the time when timber stocks are not sufficient to maintain a constant timber production (Figure \@ref(fig:illust-sust-log)).
<!-- , with a 5% tolerance. -->
...
graph <- ggplot(dftsust, aes(y=med_tsust, x = med_prodi/1e6)) +
  ...
  labs(x = bquote("Annual timber production (M"* m^3*.*yr^-1*")"), 
       y = "Duration of sustained production (yrs)", 
       color = "Logging rules") + 
