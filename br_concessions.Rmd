...
After 200 years, `r nrow(sust_simus)` (out of `r nrow(simulations)`) kept a constant total volume production. The maximum volume produced is `r signif(max(sust_prodF$medC)/1e3, 3)` km$^3$.ha$^{-1}$ in current forest concessions and `r signif(max(sust_prodF$medF)/1e6, 3)` Mm$^3$.ha$^{-1}$ in all potential concessions (Flonas). 
