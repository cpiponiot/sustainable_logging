...
omega_t = function(X) {
  ...
  t = seq.int(t1 + 1, t1 + max(logCycle))
  ord_t = floor(logCycle - t1 + 1)
  
  V = ag / th * (1 - (th * exp(-bg * t) - bg * exp(-th * t)) / (th - bg)) -
    am / th * (1 - (th * exp(-bm * t) - bm * exp(-th * t)) / (th - bm))
  
  dVG = ag * bg / (th - bg) * (exp(-bg * t) - exp(-th * t)) + am * (1 -
                                                                      (th * exp(-bm * t) - bm * exp(-th * t)) / (th - bm))
  ...
  for (i in 1:(length(t) - 1)) {
    ...
    om2 = min((om[i] * V[i] + dVG[i] * (pR[i] * omR + (1 - pR[i]) * om[i]) - dVM[i] *
                 om[i]) / V[i + 1], 1)
    # if (V[i+1] == 0) om2 = omR
    om = c(om, om2)
  }
