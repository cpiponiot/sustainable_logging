...
if (rerun) {
  ...
  recovChangOmega <- parSapply(cl, 1:nrow(dfVariables), function(i) {
    ...
    recov = timbRecovery(timeLength, logIntensity = dfVariables$logIntensity[i], 
                         ...
                         area = df_area$areaTot*100*df_area$pArea)
    return(recov)
...
ggplot(volumeVarAm_ann, aes(x=t, y = med/1e6, ymin = inf/1e6, ymax = sup/1e6, colour = offSet, fill = offSet)) + geom_line() + 
  facet_grid( scenario ~ omega0, scales = "free")  + geom_ribbon(alpha = 0.2, colour = NA)

ggplot(vextVarAm, aes(x=(ncycle-1)*logCycle, y = med/logCycle, ymin = inf/logCycle, ymax = sup/logCycle, 
