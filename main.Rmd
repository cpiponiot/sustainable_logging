...
if (rerun) {
  ...
  clusterExport(cl, varlist = c("timbRecovery", "dfVariables", "timeLength", "areaLogging_melt"))
...
df_volTot = recov[[1]][,.(inf = sum(inf), med = sum(med), sup = sum(sup)) , .(t)]
ggplot(df_volTot, aes(x=t, y = med, ymin = inf, ymax = sup)) + geom_line() + geom_ribbon(alpha = 0.2)


df_vext = recov[[2]][,.(inf = sum(inf), med = sum(med), sup = sum(sup)) , .(ncycle)]
ggplot(df_vext, aes(x=ncycle, y = med, ymin = inf, ymax = sup)) + geom_line() + geom_ribbon(alpha = 0.2)
