...
if (!file.exists("cache/timeToEsgote_BR.Rdata")) {
  ...
  save(vextVarBr, vextVarBr, file = "cache/production_concessionsBR.Rdata")
...
volumeVarBr_ann$logCycle = factor(volumeVarBr_ann$logCycle)
levels(volumeVarBr_ann$logCycle) = paste("Cycle:", levels(volumeVarBr_ann$logCycle), "yrs")

volumeVarBr_ann$ppf = factor(volumeVarBr_ann$ppf)
levels(volumeVarBr_ann$ppf) = c("Currently available","All unprotected")

ggplot(volumeVarBr_ann, aes(x=t, y = med/1e6, ymin = inf/1e6, ymax = sup/1e6, colour = paste(vext, logCycle), fill = logCycle)) + geom_line() +
  facet_grid( ppf ~ omega0, scales = "free")  + geom_ribbon(alpha = 0.2, colour = NA) + 
  labs(x = "Time (yrs)", y = "Timber volume stocks (Mm3)") + theme(legend.position = "top")
```
...
vextVarBr_ann$logCycle = factor(vextVarBr_ann$logCycle)
levels(vextVarBr_ann$logCycle) = paste("Cycle:", levels(vextVarBr_ann$logCycle), "yrs")

vextVarBr_ann$ppf = factor(vextVarBr_ann$ppf)
levels(vextVarBr_ann$ppf) = c("Currently available","All unprotected")

ggplot(vextVarBr_ann, 
