...
dfVariables = expand.grid(logCycle = c(15, 35, 65), 
                          vext = c(10, 20, 30), 
                          ppf = c("current", "potential"), ## change this later when we have all concession area types (future, potential)
                          omega0 = seq(0.2, 1, 0.2))

if (!file.exists("cache/production_concessionsBR.Rdata")) {
  ## evaluate local pre-logging timber volume (m3/ha)     
  load("data/paramStan_maxL.Rdata")
  load("data/FORMINDVariables_maxL.Rdata")
  load("data/stemMortKrig.Rdata")
  ...
  spatVariables[, vol0 := volume(t = mat0, ag = aG_FORMIND, am = aM,
                                 bg = bP, bm = bM, th = theta)]
  spatVariables <- merge(spatVariables, areaLoggingBR, by = c("long", "lat"), all = TRUE)
  spatVariables <- melt(data.table(spatVariables), measure.vars = c("pConcess", "pFlona"),
                        variable.name = "ppf", value.name = "pArea", variable.factor = TRUE)
  spatVariables <- subset(spatVariables, pArea > 0)
  levels(spatVariables$ppf) = c("current", "potential")
  ...
  clusterExport(cl, varlist = c("timbRecovery", "dfVariables", "timeLength", "areaLogging_melt", "spatVariables", "logIntensity", "rtruncnorm"))
  
  recovChangOmega <- parSapply(cl, 1:nrow(dfVariables), function(i) {
    ## subset spatialvariables to get only ppf
    dfSites = subset(spatVariables, ppf == dfVariables$ppf[i])
    
    recov = timbRecovery(timeLength = timeLength,
                         logIntensity = dfVariables$vext[i],
                         logCycle = dfVariables$logCycle[i], 
                         omega0 = dfVariables$omega0[i],
                         longitude = dfSites$long, 
                         latitude = dfSites$lat,
                         area = dfSites$areaTot*dfSites$pArea,
                         dti = NA,
                         uncertainties = TRUE)
    return(recov)
  ...
  save(volumeVarBr, vextVarBr, file = "cache/production_concessionsBR.Rdata")
  
} else { load("cache/production_concessionsBR.Rdata") }
...
volumeVarBr_ann$omega0 = factor(volumeVarBr_ann$omega0)
levels(volumeVarBr_ann$omega0) = paste0("omega0 = ",as.numeric(levels(volumeVarBr_ann$omega0))*100,"%")

volumeVarBr_ann$logrules <- paste("Cycle:", volumeVarBr_ann$logCycle, "yrs; Intensity:", volumeVarBr_ann$vext, "m3/ha")

volumeVarBr_ann$ppf = factor(volumeVarBr_ann$ppf)
levels(volumeVarBr_ann$ppf) = c("Current concessions","All Flonas")

ggplot(volumeVarBr_ann, aes(x=t, y = med/1e6)) + 
  ...
  labs(x = "Time (yrs)", y = "Timber volume stocks (Mm3)") + 
  theme_bw() + 
...
vextVarBr_ann$omega0 = factor(vextVarBr_ann$omega0)
levels(vextVarBr_ann$omega0) = paste0("omega0 = ", as.numeric(levels(vextVarBr_ann$omega0))*100,"%")

vextVarBr_ann$logrules <- paste("Cycle:", vextVarBr_ann$logCycle, "yrs; Intensity:", vextVarBr_ann$vext, "m3/ha")

vextVarBr_ann$ppf = factor(vextVarBr_ann$ppf)
levels(vextVarBr_ann$ppf) = c("Currently available","All unprotected")

ggplot(vextVarBr_ann, aes(x=t, y = med/1e6)) + 
  geom_line(aes(colour = logrules)) +
  geom_ribbon(alpha = 0.2, colour = NA, 
              aes(fill = logrules, 
                  ymin = inf/1e6, 
