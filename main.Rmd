...
if (!file.exists("cache/timeToEsgote_BR.Rdata")) {
  ...
  dfVariables$idVar = 1:nrow(dfVariables)
  volumeVarBr = merge(volumeVarBr, dfVariables[,-"id"], by = "idVar")
  
  vextVarBr = do.call(rbind, lapply(1:nrow(dfVariables), function(i) data.table(recovChangOmega[[2,i]], idVar = i)))
  dfVariables$idVar = 1:nrow(dfVariables)
  vextVarBr = merge(vextVarBr, dfVariables[,-"id"], by = "idVar")
  
  save(vextVarBr, vextVarBr, file = "cache/timeToEsgote_BR.Rdata")
...
ggplot(volumeVarBr_ann, aes(x=t, y = med/1e6, ymin = inf/1e6, ymax = sup/1e6, colour = logCycle, fill = logCycle)) + geom_line() +
  facet_grid( ppf ~ omega0, scales = "free")  + geom_ribbon(alpha = 0.2, colour = NA) + 
  labs(x = "Time (yrs)", y = "Timber volume stocks (Mm3)") + theme(legend.position = "top")
```
